...
namespace Pathfindax.Duality.Test
{
    ...
    public class NodeGridRayCasterTest
    {
        ...
        public void RayCast_DualityEnvironment_Passes()
        {
            ...
            body.AddShape(new ChainShapeInfo(new[] { new Vector2(0, 0), new Vector2(1f, 0) }));
            scene.AddObject(obj);
            var hits = new RawList<RayCastData>();

	        scene.Physics.RayCast(new Vector2(0.5f, 1f), new Vector2(0.5f, -1f), hitData => hitData.Fraction, hits);
            Assert.Single(hits);
        }
    }
}
