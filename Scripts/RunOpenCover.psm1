function RunOpenCover ([Parameter(Mandatory = $true)][string]$coverageFolder,
    ...
    $testdlls = (Get-ChildItem -Path $buildOutputFolder -Filter '*Test.dll*' -Recurse).FullName # Grab all test dll's in the buildoutput folder
    $filter = '+[Pathfindax*]* -[Pathfindax*.Test]*' # This will determine what will be included in the results
